...
namespace :hacker_news do
  ...
  task :fetch, %i[] => :environment do
    ...
    loop do
      ...
      if stash
        if stash.publisher_key == Fetchers::HackerNews::ROOT_PUBLISHER_KEY
          next if today.day > 7
      ...
      case response[:type]
      when 'comment'
        if response[:time] && response[:text]
      ...
      when 'story'
        if response[:time] && response[:kids].present? && response[:title]&.match?(/Ask HN: Who is hiring\?/)
          published_at = Time.at(response[:time])
          ...
          (response[:kids] || []).each do |child|
            pool[child.to_s] = 'comment'
          end
        end

      else
        if response[:created] && response[:submitted].present?
          ...
          PublisherStash.upsert(key, data)
          (response[:submitted] || []).each do |child|
            pool[child.to_s] = 'story'
          end
        end
      end
    end
